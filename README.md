...
### Linter installation
Before using this plugin, you must ensure that `swaglint` is installed on your system. To install `swaglint`, do the following:

1. Install [Node.js](http://nodejs.org) (and [npm](https://github.com/joyent/node/wiki/Installing-Node.js-via-package-manager) on Linux).

1. Install `swaglint` by typing the following in a terminal:
   ```
   npm install -g swaglint
   ```
...
Once you have installed and configured `swaglint`, you can proceed to install the SublimeLinter-contrib-swaglint plugin if it is not yet installed.
...
1. When the plugin list appears, type `swaglint`. Among the entries you should see `SublimeLinter-contrib-swaglint`. If that entry is not highlighted, use the keyboard or mouse to select it.
